...
let menuName = categories.map((category => { return `${category}` }))
for (i = 0; i < menuName.length; i++) {
  const btn = document.createElement("button");
  btn.textContent = menuName[i];
  ...
  btn.onclick = function (e) {
    if (e.target.textContent === "All") {
      ...
      const filteredMenu = menu.filter(item => item.category === e.target.textContent)
      renderMenuItems(filteredMenu);
    }
  }
  document.getElementsByClassName("btn-container")[0].appendChild(btn);
}
...
function renderMenuItems(menuItems) {
  ...
  menuItems.map(item => {
    ...
    titleDiv.innerHTML = `<h4>${item.title}</h4><h4 class="price">${item.price}</h4>`
    const menuTextDiv = document.createElement("div");
    menuTextDiv.className = "menu-text";
    menuTextDiv.textContent = item.desc;
    ...
    menuInfoDiv.appendChild(titleDiv);
    menuInfoDiv.appendChild(menuTextDiv);
    menuDiv.appendChild(menuInfoDiv);
    fragment.appendChild(menuDiv);
  })
